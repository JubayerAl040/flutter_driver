...
getAutoAddress(input, sessionToken, lat, lng) async {
  ...
  try {
    if (userDetails['enable_country_restrict_on_map'] == '1' &&
      ...
      response = await http.get(Uri.parse(
          'https://nominatim.openstreetmap.org/search?q=$input&limit=5&format=json'));
    }
    if (response.statusCode == 200) {
      if (mapType == 'google') {
        addAutoFill = jsonDecode(response.body)['predictions'];
        for (var element in addAutoFill) {
        ...
        addAutoFill = jsonDecode(response.body);
        for (var element in addAutoFill) {
          if (storedAutoAddress
              .where((e) =>
                  e['description'].toString().toLowerCase() ==
...
getHistory(id) async {
  ...
  try {
    ...
    var response;
    if (historyFiltter == '') {
      response = await http.get(Uri.parse('${url}api/v1/request/history$id'),
          headers: {'Authorization': 'Bearer ${bearerToken[0].token}'});
    } else {
